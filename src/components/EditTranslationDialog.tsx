...
export function EditTranslationDialog({
  ...
  const validNew =
    trimmed.length > 0 &&
    trimmed.length <= MAX_TRANSLATION_LENGTH &&
    trimmed !== currentText.trim();

  async function save() {
    setBusy(true);
    setError(null);
    const r = await fetch(`/api/messages/${messageId}/translation`, {
      method: "PATCH",
      cache: "no-store",
      headers: {
    ...
    setBusy(false);
    if (!r.ok) {
      setError(ru.inbox.message.translationSaveError);
      return;
    }
    await onSaved();
    onClose();
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 animate-fade-in">
      <div className="bg-tg-bg-section text-tg-text w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-5 shadow-2xl animate-slide-up space-y-3">
        <h2 className="font-semibold tracking-tight">
          {ru.inbox.message.translationDialogTitle}
        </h2>
        <p className="text-xs text-tg-text-hint">
          {ru.inbox.message.translationDialogHint}
        </p>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          >
            {ru.inbox.message.translationCancelButton}
          </button>
          <button
            type="button"
          ...
          >
            {busy ? <Spinner /> : ru.inbox.message.translationSaveButton}
          </button>
        </div>
      </div>
    </div>
